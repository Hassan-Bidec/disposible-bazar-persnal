...
// ─── CustomizationDetails Client Component ────────────────────────────────────
// Accepts optional `initialData` from SSR page.js.
// When initialData is provided, skips the client-side fetch entirely.
// ─────────────────────────────────────────────────────────────────────────────

export default function CustomizationDetails({ initialData = null }) {
    console.log("initialDataaaaaaaa", initialData)
    ...
    // Extract slug from pathname (used for client-side fetch fallback)
    const pathParts = (pathname || '').split('/customization/');
    const id = pathParts.length > 1 ? pathParts[1].replace(/\/$/, '/') : null;

    // ─── Helper: apply API response data to all state ─────────────────────────
    ...
    console.log("initialData SEO:", initialData?.seoMetadata);
            applyProductData(initialData);
        }
    }, [initialData]);

    // ─── Client-side fetch fallback (only when no SSR data) ──────────────────
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                applyProductData(response.data.data);
    ...
    const handleAddCart = async (product) => {
        ...
        const lid_Price = lid_price_per_piece;
        const custom_Note = customizeDetail || null;
        const option_Price = printing_price_per_piece;
        const order_limit = product?.order_limit || 1000;
        const packaging_options = {
            ...selectedOption,
            total_price: total_printing_price
        };
        ...
        const payload = {
            product_id, product_name, product_quantity, pack_size, total_pieces, printingPrice,
            price_per_piece, lid_price_per_piece, printing_price_per_piece,
            total_base_price, total_lid_price, total_printing_price,
            product_total: product_total_final, order_limit
        };

        addToCart(
            product_id, product_name, product_quantity, pack_size, total_pieces,
            price_per_piece, product_img, product_total_final, product_variants,
            printing_price, product_color, product_size, logo, product_options,
            product_lids, lid, lid_Price, customizeDetail, option_Price,
            null, order_limit, packaging_options,
        );
    ...
    return (
        <div className="relative py-32 px-10 text-white overflow-hidden">
                    ...
                    <div className="lg:w-[55%] flex flex-col gap-5 text-white h[300px]">
                        ...
                        <form onSubmit={handleSubmit} className='w-3/4 flex flex-col gap-5'>
                            <div className="flex mdflex-row flex-wrap gap-3">
                                {productLid?.length > 0 && (
                                    <div className="relative">
                                        <h3
                                            ...
                                            <p>{selectedLid ? selectedLid : 'No Lids'}</p>
                                ...
                                <div className="relative">
                                    <h3
                                        ...
                                        <p>{selectedPackSize ? selectedPackSize : 'Pieces'} Pcs</p>
                                        ...
                                        <div className="md:w-24 w-40 rounded-md my-2 h-32 absolute z-10 overflow-auto bg-white text-black">
                                            {productVariants.map((variant) => (
                                                <div
                                                    ...
                                                    className="p-2 cursor-pointer hover:bg-gray-200"
                                                    onClick={() => {
                                                        setSelectedPackSize(variant.pack_size);
                                ...
                                <div className="relative">
                                    <h3
                                        ...
                                        <p>{selectedOption ? `${selectedOption.side_option} - ${selectedOption.print_location}` : 'Packaging Option'}</p>
                                        ...
                                        <div className="md:w-52 w-40 rounded-md my-2 h-32 absolute z-10 overflow-auto bg-white text-black">
                                            {productPackageOptions.map((variant) => (
                                                <div
                                                    ...
                                                    {variant.side_option + '-' + variant.print_location}
                                                </div>
                                            ))}
                                        </div>
                                    )}
                            ...
                            <div className="w-full md:md:w-3/2 w[23.5rem]">
                                <textarea
                                    className='w-full rounded-lg p-2 outline-none bg-transparent border border-[#1E7773] resize-none'
                                    rows={2}
                                    placeholder='Additional Customization'
                                    value={customizeDetail}
                                    onChange={(e) => setCustomizeDetail(e.target.value)}
                                ></textarea>
                            </div>
                            ...
                            <div className='flex flex-row gap-3'>
                                ...
                                <button
                                    className='p-2 pt-3 bg-[#1E7773] w-full lg:text-[15px] font-bazaar text-xs rounded-md'
                        ...
                        <div>
                            ...
                            {Number(selectedLidPrice) > 0 && (
                                <div className="flex items-center gap-2 py-2">
                                    <h3 className="text-[13px] font-semibold text-white">Lid Price :</h3>
                                    <p className="text-[13px] font-semibold text-white">{Number(selectedLidPrice)}</p>
                                </div>
                            )}
                            {selectedOptionPrice ? (
                                <p>Printing Price: {Number(selectedOptionPrice) * Number(selectedPackSize || 1)}</p>
                            ) : (
                                <p className="text-gray-400 text-sm">Select a variant to see price</p>
                            )}
                            {productDetail.product?.activeDiscount && (
                                <p className='text-sm'>{Number(productDetail.product?.activeDiscount?.discount_percentage)}% OFF ( {productDetail.product?.activeDiscount?.name} )</p>
                            )}
                        </div>

                        <div className="flex flex-row md:gap-5 gap-2">
                            <button
                                className='p-2 pt-3 border-b-4 border-[#1E7773] w-32 lg:text-[15px] font-bazaar cursor-pointer text-xs'
                                onClick={() => handleWishlist(productDetail.product.id)}
                            ...
                            >
                                <FaWhatsapp className='text-[#1E7773] text-2xl' />
                                <p className="pt-2 cursor-pointer">ORDER ON WHATSAPP</p>
                            </button>
                        </div>
                    </div>
                </section>
                        ...
                        {productTextDetail === 'Description' && (
                            <div className="flex flex-col gap-2">
                                {productDetail.product?.description ? (
                                    <p dangerouslySetInnerHTML={{ __html: productDetail.product.description }} className="text-md" />
                                ) : (
                        ...
                        {productTextDetail === 'Additional information' && (
                            <div className="flex flex-col gap-2">
                                {productDetail.product?.additional_information ? (
                                    <p dangerouslySetInnerHTML={{ __html: productDetail.product.additional_information }} className="text-md" />
                                ) : (
            ...
            {/* Background Images */}
            <img data-aos="fade-left" className="absolute top-[44rem] right-0 md:w-28 w-16" src={`${Image_Url}plateRight.svg`} alt="Plate" />
            <img data-aos="fade-right" className="absolute top-[100rem] left-0 lg:w-16 w-8" src={`${Image_Url}leftCup.svg`} alt="Plate" />
